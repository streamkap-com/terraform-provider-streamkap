...
- **Create returns 422 "already exists"** when a non-deleted record with the same `{tenant_id, name}` exists upstream. The v2 client surfaces this as an error from `doRequest` — there's no client-side adopt-by-name behavior here, so the user has to delete the conflicting record (or rename) before the apply succeeds.
