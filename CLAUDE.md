...
- **`main`** — v3.x stable. Default target for new feature work. Details deliberately omitted here so this file describes the v2 line.
...
Terraform provider for Streamkap (data streaming platform). Built with Terraform Plugin Framework. Provider address: `streamkap-com/streamkap`.
...
# Testing
TF_ACC=1 go test ./internal/provider -v -timeout 120m # Acceptance tests; requires credentials
...
# Single test
TF_ACC=1 go test ./internal/provider -v -run '^TestAccSourcePostgreSQLResource$'
```
...
provider_installation {
  dev_overrides {
    "streamkap-com/streamkap" = "$GOBIN_PATH"  # your $GOPATH/bin
  }
  direct {}
}
```
...
The v2 API client only exposes Create/Get/Update/Delete for sources, destinations, and pipelines — there are no `List*` methods on this branch. Topic API surface is narrower still: `GetTopic` and `UpdateTopic` only; topics are created and deleted as a side effect of pipeline lifecycle.
...
- **`?secret_returned=true`** is appended to every Source, Destination, and Pipeline operation (Create / Get / Update / Delete) so sensitive fields round-trip in responses. If you add a new operation against any of those endpoints, include the same query parameter or the response will omit secrets.
- **Create returns 422 "already exists"** when a non-deleted record with the same `{tenant_id, name}` exists upstream. The v2 client surfaces this as an error from `doRequest` — there's no client-side adopt-by-name behavior here, so creation fails. If the existing resource should be managed by this configuration and is absent from state, verify its ID and import it. Do not delete an existing resource merely to resolve a name conflict.
...
- Provider address: `streamkap-com/streamkap`. Go 1.x as pinned in `go.mod`. Terraform Plugin Framework.
