...
- **v2.x (stable)** — what users should pin in production: `version = "~> 2.1"`. Lives on the `v2` branch.
- **v3.x (beta)** — new resources (`streamkap_destination_weaviate`, `streamkap_kafka_user`, `streamkap_client_credential`, `streamkap_roles` data source) plus schema and API changes. Deprecated v2 aliases remain supported through v3.x. Lives on the `main` branch. **Do not use in production yet.**

`main` now contains v3 beta; `v2` retains the independent v2 history. The branch reorganization does not make v3 stable. If you're consuming the provider today, target v2.x; if you're contributing, see [Branches](#branches) below.

**Contributing on `main`:** files under `internal/generated/` are written by `tfgen` (header: `// Code generated by tfgen. DO NOT EDIT.`) and are rewritten on every regen. If you spot a bug in generated code, fix the generator (`cmd/tfgen/*.go` or `cmd/tfgen/overrides.json`) and re-run `make generate` (not `go generate ./...`, which renders docs against the previous schema) — don't patch the generated file directly, or the next regen will revert your fix. Full procedure is under [Fix the generator, not the generated output](#fix-the-generator-not-the-generated-output) below.
...
- **`main`** — v3.x beta. Default target for features and PRs.
- **`v2`** — v2.x stable maintenance line. Bug and security fixes only.

Keep the histories independent. A fix for v2 users belongs on `v2`; a shared
fix needs a separate commit on each line. Do not merge `main` and `v2`. Cut
feature branches from the line you are targeting.

Pull and confirm the branch before starting (`main` = v3, `v2` = v2). Do not
assume the working tree is current.

### Release flow

"Release the next beta" = use `main`, pull, verify a clean tree and passing checks, and select the next unused beta tag. **Pushing the tag triggers the public release workflow** — surface the exact `git tag`/`git push` commands and STOP for explicit approval. Never push tags or branches unprompted.
...
The branch reorganization is complete: former `main` is `v2`, former
`develop` is the default `main`, and both histories are preserved. v3 remains
beta until a stable release is published. Public tags require separate approval.

1. Verify the intended release commit and both lines' release workflows.
   Workflows run from the tagged commit; changes on `main` do not update `v2`.
...
3. Prepare the next unused beta's changelog and pins, run preflight and checks,
...
For clones that still use the old names, preserve local commits, rename the old
local `main` to `v2` and `develop` to `main`, then fetch/prune, set each upstream
to its matching remote branch, and run `git remote set-head origin -a`. Check
for existing local branches before renaming; do not overwrite local history.
