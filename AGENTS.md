...
- `"produced an unexpected new value: was cty.StringVal(\"\"), but now null"` is an `Optional+Computed` echo mismatch — the API echo differs from the default. It is not just `insert_static_*`/static-transform fields: it also hits deprecated aliases and placeholder `<...>` defaults (see `HasPlaceholderDefault`). When you touch one, audit **all** siblings of that class across **every** connector — past fixes only patched a subset. Defaulted `Optional+Computed` strings are refilled when the API echoes null — from the plan on Create/Update and from prior state on Read (`shared.DefaultedStringAttrNames`) — because the backend drops a conditional field whose gate is unmet, e.g. `iceberg_catalog_scope` unless `iceberg_catalog_auth_mode` is `oauth2`. A non-null echo that differs from the plan is left alone so it still fails loudly.
