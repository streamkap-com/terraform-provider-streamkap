...
- Write-only secrets and rotation: HOLDS.
  ...
  - `TestPlanSecretOps` is table-driven: version below 1, decreased version, increased version with no rows, unknown key, pointer outside the type's secret paths, pointer of another type, value and clear together, neither, and the two-level keyed pointer.
...
- Released v3 compatibility: HOLDS.
  - `TestReleasedStateDecodesAndConverges`: a state encoded with the released schema's own attribute set (no chain keys) goes through the prototype's `UpgradeResourceState`, decodes with `smt_chain` and `smt_secrets` null and the flat values intact, refreshes unchanged, and plans with the chain still null and the flat values unmoved.
  - `TestResource_FlatSurfaceUnchanged` under terraform: flat HCL using canonical and deprecated-alias attributes applies, has no chain in state, and re-plans empty.
...
- Reading secrets back is impossible by construction, so a rotation done outside Terraform is invisible and produces no diff. Only the version is stateful.
...
- `go test ./internal/smtproto/... -v -count=1`: 23 tests, 49 subtests, 0 failures, about 7.5s (four cases drive terraform).
