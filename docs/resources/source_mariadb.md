...
description: |-
  ...
  This resource creates and manages a MariaDB source for Streamkap data pipelines. Use with streamkap_pipeline to connect sources to destinations.
...
- `database_ssl_mode` (String) Enables SSL/TLS in a specific mode. 'disable' = unencrypted connection. 'trust' = encrypted, no certificate verification. Defaults to `trust`. Valid values: `disable`, `trust`.
- `heartbeat_data_collection_schema_or_database` (String) The database containing the 'streamkap_heartbeat' table. This table must be created before enabling heartbeat — see the Streamkap documentation for setup instructions.
