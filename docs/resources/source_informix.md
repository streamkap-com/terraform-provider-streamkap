...
page_title: "streamkap_source_informix Resource - terraform-provider-streamkap"
subcategory: ""
description: |-
  Manages a Informix source connector.
  This resource creates and manages a Informix source for Streamkap data pipelines. Use with streamkap_pipeline to connect sources to destinations.
  Documentation https://docs.streamkap.com/streamkap-provider-for-terraform
---

# streamkap_source_informix (Resource)
...
- `database_dbname` (String) The name of the Informix database from which to stream the changes.
- `database_hostname` (String) IP address or hostname of the Informix database server.
- `database_password` (String, Sensitive) Password to access the Informix database.

**Security:** This value is marked sensitive and will not appear in CLI output or logs.
- `database_user` (String) Username to access the Informix database.
...
- `binary_handling_mode` (String) Specifies how the data for binary columns e.g. blob, byte, text should be represented. This setting depends on what the destination is. See the documentation for more details. Defaults to `bytes`. Valid values: `bytes`, `base64`, `base64-url-safe`, `hex`.
- `cdc_buffersize` (Number) Size of the read buffer for receiving CDC events from the Informix server, in bytes. Defaults to `65536`.
- `cdc_timeout` (Number) Timeout (seconds) the CDC engine will wait before breaking out of a blocking read to check for shutdown signals. Defaults to `5`.
...
- `database_port` (Number) Informix SQLI listener port. For example, 9088. Defaults to `9088`.
...
- `signal_data_collection_schema_or_database` (String) Path to the signal table as schema.table (e.g., 'myschema.streamkap_signal'). The database name will be added automatically. This table is used for incremental snapshotting. Defaults to `streamkap`.
...
```shell
# Source Informix can be imported by specifying the identifier.
terraform import streamkap_source_informix.example-source-informix 00000000000000000000000000
```
