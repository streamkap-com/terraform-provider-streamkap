...
page_title: "streamkap_source_zendesk_webhook Resource - terraform-provider-streamkap"
subcategory: ""
description: |-
  Manages a Zendesk Webhook source connector.
  This resource creates and manages a Zendesk Webhook source for Streamkap data pipelines. Use with streamkap_pipeline to connect sources to destinations.
  Documentation https://docs.streamkap.com/streamkap-provider-for-terraform
---

# streamkap_source_zendesk_webhook (Resource)
...
- `camel_source_payload_router_unknown_type_behavior` (String) Behavior when an unknown Zendesk event type is encountered. Defaults to `DEFAULT_TOPIC`. Valid values: `DEFAULT_TOPIC`, `SKIP`, `FAIL`.
- `camel_source_payload_router_unknown_type_default_topic` (String) Topic name for unknown event types (used when Unknown Event Behavior is DEFAULT_TOPIC). Defaults to `unknown`.
...
```shell
# Source Zendesk Webhook can be imported by specifying the identifier.
terraform import streamkap_source_zendesk_webhook.example 00000000000000000000000000
```
