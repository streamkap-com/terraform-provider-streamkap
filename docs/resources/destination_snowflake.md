...
- `snowflake_streaming_iceberg_enabled` (Boolean) Specifies whether the connector should write into a pre-existing Snowflake-managed Iceberg table instead of a standard Snowflake table (applies to `append` only). The target table must already exist in Snowflake before enabling this option. Defaults to `false`.
...
- `transforms_key_to_value_fields_include_list` (String) Comma-separated list of key fields to copy into each record's value so they land as columns at the destination. Use `*` to copy all key fields, or `<topic>.<field>` patterns to scope by topic and field name (e.g. `*.pk,*.sk` where `*` matches any topic). Leave empty to disable.
